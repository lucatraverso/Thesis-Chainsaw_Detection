...
def waveform_to_log_mel_spectrogram_patches(waveform, params):
  ...
  with tf.name_scope('log_mel_features'):
    ...
    #AUMENTO DE DATOS
    if np.random.rand() < params.TM:
      w = np.random.randint(9, 24)
      log_mel_spectrogram = tfio.audio.time_mask(log_mel_spectrogram, w)
      #for i in range(tf.shape(features)[0].numpy()):
      #  w = np.random.randint(9, 24)
      #  features[i] = tfio.audio.time_mask(features[i], w)
    if np.random.rand() < params.FM:
      w = np.random.randint(9, 20)
      log_mel_spectrogram = tfio.audio.freq_mask(log_mel_spectrogram, w)
#      for i in range(tf.shape(features)[0].numpy()):
#        w = np.random.randint(10, 20)
#        features[i] = tfio.audio.freq_mask(features[i], w)
...
def pad_waveform(waveform, params):
  ...
  ### AUMENTO DE DATOS
  if np.random.rand() < params.PS:
    padded_waveform = pitch_shift(padded_waveform, np.random.randint(-4, 4))
  if np.random.rand() < params.BN:
    padded_waveform = add_white_noise(padded_waveform, 10)
  ### FIN AUMENTO
